...
final class VpnConnectionController {
    ...
    static Intent startIntent(Context context, SharedPreferences preferences) {
        return new Intent(context, AutoConnectVpnService.class)
                .setAction(AutoConnectVpnService.ACTION_START)
    ...
    static void disconnect(Context context) {
        context.startService(new Intent(context, AutoConnectVpnService.class).setAction(AutoConnectVpnService.ACTION_STOP));
    }
